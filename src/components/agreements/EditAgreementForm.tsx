...
export default function EditAgreementForm({ initialData }: { initialData: Agreement }) {
  const router = useRouter();
  const { toast } = useToast();
  const [loading, setLoading] = useState(false);
  const [expiryPreview, setExpiryPreview] = useState<string | null>(null);

  const formattedStartDate = new Date(initialData.startDate).toISOString().split("T")[0];
  ...
  const {
    ...
    defaultValues: {
      ...
      tenantMobile: initialData.tenantMobile,
  ...
  const onSubmit = async (values: AgreementFormValues) => {
    setLoading(true);
    const res = await updateAgreement(initialData.id, {
      ...values,
    ...
    if (res.success) {
      toast("Agreement updated successfully!");
      router.push(`/agreements/${initialData.id}`);
    } else {
      toast(res.error || "Failed to update agreement", "error");
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-6">
        <Link
          href={`/agreements/${initialData.id}`}
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            Modify contract parameters and update details
          </p>
        </div>
      </div>
